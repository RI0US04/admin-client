 ...
 * @param {string} userID
 */
export async function revokeAdminTOTP(userID) {
    try {
        const response = await fetch(`${ADMIN_HTTP_API_ENDPOINT}/api-adpanel/disable-admin-totp/`, {
            method: 'POST',
            headers: {
        ...
        if (response.ok || (response.status >= 400 && response.status < 500)) {
            ...
            throw new Error('Revoke admin TOTP failed');
        }
    } catch (error) {
        // @ts-ignore
        throw new Error('Error Request revoke admin TOTP: ' + error.message);
    }
}